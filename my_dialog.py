    ...
    def __init__(self, browser, parent=None) -> None:
        ...
        if len(common_fields) < 1:
            ...
            qt.QMessageBox.critical(
                mw,
                "Error",
                f"The chosen notes only share a single field in common '{list(common_fields)[0]}'. You need an front and back field",
            )

        self.front_combo = qt.QComboBox()
        self.back_combo = qt.QComboBox()

        last_front_field = config.get("last_front_field") if config else None
        last_back_field = config.get("last_back_field") if config else None
        front_field_index = 0
        back_field_index = 0
        i = 0
        ...
        front_lang_index = 0
        back_lang_index = 0
        last_front_lang_field = config.get("last_front_lang_field") if config else None
        last_back_lang_field = config.get("last_back_lang_field") if config else None
        ...
        delay = config.get("delay") if config else "0"
        ...
        self.pattern_input.setValidator(
            qt.QRegularExpressionValidator(qt.QRegularExpression("^[fb]*$"), self)
        )
        pattern = config.get("pattern") if config else ""
        ...
        add_bip = config.get("add_bip") if config else "false"
        ...
        directory = config.get("directory") if config else ""
